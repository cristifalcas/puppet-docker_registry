...
start() {
    ...
    if ! [ -f $pidfile ]; then
        ...
        $exec /usr/bin/gunicorn --access-logfile - --error-logfile - --debug --max-requests 100 --graceful-timeout 3600 -t 3600 -k gevent -b ${REGISTRY_ADDRESS}:${REGISTRY_PORT} -w $GUNICORN_WORKERS --preload docker_registry.wsgi:application &>> $logfile &
        pid=$!
        echo $pid > $pidfile
        touch $lockfile
        success
